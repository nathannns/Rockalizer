...
void TremoloModule::process (juce::AudioBuffer<float>& buffer)
{
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
    {
        ...
        // >1 -- a convex curve that stays gentle near the zero-crossing then
        // accelerates hard into the trough, mirroring gm collapsing near
        // cutoff. Away from cutoff (x>0) uses an exponent <1 -- a concave
        // curve that rises quickly off zero then flattens out approaching
        // the tube's normal ceiling.
