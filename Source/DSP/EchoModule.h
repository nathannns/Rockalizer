...
// one-pole "Tone" and plain tanh saturator this module used to have.
//
...
// ported byte-for-byte with only the class name differing. Threadline names
// it SpaceEchoModule because it already uses EchoModule for the unrelated
// "Plexer" EP-3 delay; Rockalizer has no such collision, so it keeps the
// original EchoModule name.
class EchoModule
{
public:
