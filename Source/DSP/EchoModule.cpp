...
void EchoModule::setParameters (float timeMs, float repeats, float bassPercent, float treblePercent,
...
void EchoModule::updateShelfCoefficients()
{
...
float EchoModule::processHysteresisDrive (int channel, float sample, float driveAmount)
{
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
    {
        ...
        else
        {
            for (int channel = 0; channel < channels; ++channel)
            {
                ...
                // header documentation. Previously tone/drive was only
