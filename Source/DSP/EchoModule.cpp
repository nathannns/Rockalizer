...
void EchoModule::reset()
{
    ...
    writeIndex = 0;
    lfoPhase = 0.0f;
    delaySamples.setCurrentAndTargetValue (sampleRate * 0.375);
                                ...
                                float drive, float mix, bool enabled, int patternIndex)
{
    ...
    feedback = juce::jlimit (0.0f, 0.94f, repeats * 0.0094f);
    toneCutoff = juce::jlimit (1200.0f, 14000.0f, toneHz);
    wobbleDepth = juce::jlimit (0.0f, 1.0f, wobble * 0.01f);
    driveAmount = juce::jlimit (0.0f, 1.0f, drive * 0.01f);
...
float EchoModule::readDelay (int channel, float distance) const
{
    ...
    while (position >= size) position -= static_cast<float> (size);
